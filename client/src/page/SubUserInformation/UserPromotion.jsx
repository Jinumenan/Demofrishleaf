    ...
    return (
        ...
        <div className='p-5'>
          ...
          <button className='w-[230px] h-[40px] bg-gray-200 rounded-2xl text-center my-3'><Link to="/userFeedback">Promotion</Link></button>
           ...
           {users.map((user, index) => (
    <div key={user._id} className='flex bg-gray-200 rounded-3xl ml-10 w-[550px] p-2 h-[250px]'>
      <div className='flex-1 flex flex-col justify-between px-2 py-4'>
        ...
        <div className='flex justify-center'>
          <button onClick={() => deleteReview(user._id)} className='bg-red-500 text-white rounded-2xl px-9 py-1 ml-[350px] mr-2'>Delete</button>
        </div>
      </div>
    </div>
  ))}
